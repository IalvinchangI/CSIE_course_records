...
class MLPlay:
    ...
    def __init__(self,ai_name, *args, **kwargs):
        ...
        # self.__store_file = collect.ML_Data(r".\data_linear_3", r"ml_data.dat", "w")
        print(ai_name)
    ...
    def update(self, scene_info, *args, **kwargs):
        ...
        if not scene_info["ball_served"]:
            if self.__scene_info["platform"][0] == 60:
        ...
        command = self.__decision(condition)
    ...
    def __platform_intersection(self, expression: numpy.ndarray) -> float:
        ...
        return ((self.world_width - distance - error_distance) if times % 2 else (distance + error_distance))
    ...
    def __collect_data(self, condition, command):
        """ collect data """
        self.__collect.append({"condition": copy.deepcopy(condition), "action": (command, self.real_x)})