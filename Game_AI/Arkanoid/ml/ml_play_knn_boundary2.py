...
class MLPlay:
    ...
    def __init__(self,ai_name, *args, **kwargs):
        ...
        self.model = collect.ML_Data(r".\model", r"ml_fit_boundary26.model").read()
        self.model2 = collect.ML_Data(r".\model", r"ml_fit_platform_B7__.model").read()
        self.name = ai_name
        print(ai_name)
    ...
    def update(self, scene_info, *args, **kwargs):
        ...
        if scene_info["ball"][1] > self.cut_offset:
            self.__transform_data()
            command = self.model2.predict(numpy.array((scene_info["ball"][0], self.__scene_info["platform"][0] + self.platform_width / 2)).reshape(1, -1))[0]
        else:
            condition = self.__dicision(self.__transform_data())
