...
static int osfs_add_dir_entry(struct inode *dir, uint32_t inode_no, const char *name, size_t name_len)
{
    ...
    dir_entry_count = parent_inode->i_size / sizeof(struct osfs_dir_entry);
    if (dir_entry_count >= MAX_DIR_ENTRIES) {  // Here, we only supprt MAX_DIR_ENTRIES entries, so there is only one block.
...
{   
    ...
    parent_inode->__i_mtime = now;  // modify means write
    inode_set_mtime_to_ts(dir, now);  // modify means write
    inode_set_ctime_to_ts(dir, now);  // record the time we change i_mtime
    mark_inode_dirty(dir);  // require system to write inode back later because the inode on disk haven't change yet
