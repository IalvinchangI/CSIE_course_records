...
public class Indexer implements Serializable {
    ...
    /** the word in corpus file name */
	private String parsingWord;

    
    /** parsingWord 的 getter */
    public String getParsingWord() {
        return this.parsingWord;
    }
 ...
 * 裝 Indexer
 */
class IndexerContainer implements Serializable {
    ...
    public int getIndex(String word, int number) {
        return Arrays.binarySearch(this.numbers.get(getIndex(word)), number);
    }
    ...
    public IndexerContainer(String fileName) {
     ...
     */
    public boolean __put(String word, int documentNumber, double value) {  // In fact, the behavior is to insert.
     ...
     * @apiNote if insert the value again, the new value will replace previous value
     */
    public boolean put(String word, int[] documentNumbers, double[] values) {  // In fact, the behavior is to insert.
        int wordIndex = __getIndex(word);
        boolean output = wordIndex >= 0;
        ...
        if (output == false) {  // add new space for new word
            ...
            if (wordIndex < this.__words.size()) {
                this.__words.add(wordIndex, word);
                this.numbers.add(wordIndex, documentNumbers);
                this.values.add(wordIndex, values);
            }
            else {  // add to end of the list
                this.__words.add(word);
                this.numbers.add(documentNumbers);
    ...
    public void transform() {
        ...
        // numbers
        this.numbers = new ArrayList<>(__numbers.size());
        for (int i = 0; i < __numbers.size(); i++) {
            numbers.add(i, new int[__numbers.get(i).size()]);
            for (int j = 0; j < __numbers.get(i).size(); j++) {
                numbers.get(i)[j] = __numbers.get(i).get(j);
            }
        }
        
        // values
        this.values = new ArrayList<>(__values.size());
        for (int i = 0; i < __values.size(); i++) {
            values.add(i, new double[__values.get(i).size()]);
            for (int j = 0; j < __values.get(i).size(); j++) {
                values.get(i)[j] = __values.get(i).get(j);
            }
        }
    }
}
