...
volatile unsigned char finish_line_counter = FINISH_LINE;
...
void main(void) {
    ...
    IPR1bits.TMR2IP = 0b0;          // Low priority (timer interrupt is high priority)
    PIR1bits.TMR2IF = 0b0;          // Clear TMR2 to PR2 Match Interrupt flag bit (if interrupt occurs, TMR2IF will become 1)
    PIE1bits.TMR2IE = 0b1;          // Enables the TMR2 to PR2 match interrupt
     ...
     * reset = 0
     */
    setup_duty_cycle(DUTY_CYCLE_NEG_90);
...
void __interrupt(__low_priority) Low_Priority_ISR_Table(void) {
    if (FINISH_INIT == 0xFF) {
        if (PIE1bits.TMR2IE && PIR1bits.TMR2IF) {
            if (finish_line_counter < FINISH_LINE) {
                Timer2_ISR();
            }
            PIR1bits.TMR2IF = 0b0;
        }
