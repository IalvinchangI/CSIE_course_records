...
enum {
    ...
    DUTY_CYCLE_ZERO = 47, 
    DUTY_CYCLE_INC_STEP = 16,  // 0.5
    DUTY_CYCLE_DEC_STEP = -16  // -0.5
};
...
//    RESET();
     ...
     * reset = 0
     */
    setup_duty_cycle(DUTY_CYCLE_ZERO);
    ...
    INTCONbits.GIE = 1;        // Enables all unmasked interrupts (because IPEN=0 and GIE=1)
...
void __interrupt() High_Priority_ISR_Table(void) {
    if (FINISH_INIT == 0xFF) {
