...
bool code_createVariable(ValueData* valueData, char* name) {
    ...
    if (valueData->valueType != OBJECT_TYPE_AUTO && valueData->valueType != srcValueType) {
        if (
            (ObjectType_isNumber(valueData->valueType) && ObjectType_isNumber(srcValueType)) ||
            (valueData->valueType == OBJECT_TYPE_NUM && ObjectType_isNumber(srcValueType)) ||
            (ObjectType_isNumber(valueData->valueType) && srcValueType == OBJECT_TYPE_NUM)
        ) {
            // I32, I64, F32, F64 are NUM
        }
        else {
            yyerrorf("變數型別 %s 與數值 %s 型別不同\n", 
                        objectType2str[valueData->valueType], objectType2str[srcValueType]);
            goto FAILED;
        }
    }
...

    {
        ...
        Object regSrc = object_nameLiteralOrLoadReg(src, regName, MAX_NAME_LENGTH);
        if (regSrc.type == OBJECT_TYPE_UNDEFINED) {
            yyerrorf("變數型別為 %s\n", objectType2str[OBJECT_TYPE_UNDEFINED]);
            goto FAILED;
        }
...
bool code_assign(Object* dest, Object* src) {
    ...
    const ObjectType destType = object_getValueType(dest);
    if (srcType != destType) {
        yyerrorf("變數型別 %s 與 %s 不同\n", objectType2str[destType], objectType2str[srcType]);
        goto FAILED;
    }
    ...
    Object regSrc = object_nameLiteralOrLoadReg(src, regName, MAX_NAME_LENGTH);
    if (regSrc.type == OBJECT_TYPE_UNDEFINED) goto FAILED;
    const char* llvmTypeName = objectType2llvmType[srcType];
    buffPrintln(&ctx->code, "store %s %s, ptr %%var.%d", llvmTypeName, regName, dest->value.symbol->index);
    if (src->type == OBJECT_TYPE_SYMBOL) object_free(&regSrc);
...
Object code_getLength(Object* obj, const YYLTYPE* loc) {
    ...
    // get array
    char regArrayName[MAX_NAME_LENGTH];
    Object regArray = object_nameLiteralOrLoadReg(obj, regArrayName, MAX_NAME_LENGTH);
    if (regArray.type == OBJECT_TYPE_UNDEFINED) {
        goto FAILED;
    }

    // create result reg
    SymbolData _resultReg = object_createRegisterSymbol(OBJECT_TYPE_I64);
    SymbolData* resultReg = cloneStruct(SymbolData, &_resultReg);
    Object result = {
        .type = OBJECT_TYPE_REGISTER, 
        .capturedIndex = -1, 
        .value.symbol = resultReg
    };

    // gen IR
    if (objType == OBJECT_TYPE_ARRAY) {
        buffPrintln(&ctx->code, "%%reg%s = call i64 @wy_rt_array_get_length(ptr %s)",
            resultReg->name, regArrayName);
    }
    else {  // objType == OBJECT_TYPE_STR
        buffPrintln(&ctx->code, "%%reg%s = call i64 @wy_rt_str_length(ptr %s)",
            resultReg->name, regArrayName);
    }

    // output
    compilerLogAt(loc, "length %s -> %s\n", object_print(obj), object_print(&result));
    object_free(obj);
    return result;
