...
Object object_nameLiteralOrLoadReg(const Object* src, char* buffer, const size_t bufferLen) {
    ...
    case OBJECT_TYPE_SYMBOL: {
        ...
        if (src->capturedIndex >= 0) {  // Closure Variable
        ...
        else if ((src->value.symbol)->funcArg == true) {  // Function Arguments
            snprintf(buffer, bufferLen, "%%var.%d", (src->value.symbol)->index);
            return (Object){OBJECT_TYPE_REGISTER, .value.symbol = symbol_clone(src->value.symbol)};
        }
        else if ((src->value.symbol)->type == OBJECT_TYPE_FUNC) {  // Function Object
            snprintf(buffer, bufferLen, "@func_%d_%d", (src->value.symbol)->funcInfo->contextId, (src->value.symbol)->index);
            return (Object){OBJECT_TYPE_REGISTER, .value.symbol = symbol_clone(src->value.symbol)};
        }
        else {  // Load symbol value to register（一般變數）
