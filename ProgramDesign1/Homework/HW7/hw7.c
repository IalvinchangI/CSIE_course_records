...
int file_address[max_file_count] = {0};  // address(index) of file in hard drive --> header
...
void delete_file(int count, int sum_size) {
	...
	for (int i = 0, index = 0; i < count; i++) {
		index = file_address[final_delete[i]];
	...
	for (int index = max_file_count - n; index < max_file_count - layer + 1; index++) {
		...
		if (layer == 1) {  // stop
			...
			for (int i = 0; i < count; i++) {
				sum += file[file_address[temp[i]]][1];
			}
