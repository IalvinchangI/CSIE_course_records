...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  size_t idx = (addr >> idx_shift) & (sets-1);
  size_t way = 0;
  if (fifos[idx].size() == ways) {  // Cache set full
    way = fifos[idx].front();
    fifos[idx].pop();
  }
  else {  // Look for an invalid line
    for (way = 0; way < ways; way++) {
      if ((tags[idx*ways + way] & VALID) == 0) break;
    }
  }
...
{
  uint64_t old_tag = 0;
  if (tags.size() == ways)  // Cache set full
  {
    auto it = tags.begin();
    size_t way = fifos[0].front();
    fifos[0].pop();
    std::advance(it, way);
    old_tag = it->second;
    fifos[0].push(way);
